...
```
if (question.contains("退货")) {    return "请在订单详情页点击申请退货"; } else if (question.contains("发货")) {    return "下单后 48 小时内发货"; } else {    return "请联系人工客服"; }
```
...
是（MIT）（[Hugging Face](https://huggingface.co/deepseek-ai/DeepSeek-R1)）
...
是（Apache-2.0，开放权重）（[Hugging Face](https://huggingface.co/Qwen/Qwen3.5-397B-A17B)）
...
是（MIT） ([Hugging Face](https://huggingface.co/zai-org/GLM-5))
...
部分开源（M1 开源 Apache-2.0；abab 多为服务型） ([MiniMax](https://www.minimaxi.com/news/minimaxm1?utm_source=chatgpt.com))
...
是（Modified MIT） ([Hugging Face](https://huggingface.co/moonshotai/Kimi-K2.5))
...
代码与长上下文强；computer-use/Agent 能力突出
...
通用 + Agent 取向；支持 Thinking in Tool-Use；性价比高
...
面向 Coding/Agent；长上下文 + 高输出上限（128K）
...
![无法获取该图片](https://oss.open8gu.com/image-20260222130016616.png)
...
- 2.
如果是 Ollama，可以通过 `ollama show --modelfile <模型名>` 查看 modelfile，看是否包含对话模板（如 `<|im_start|>`）、System Prompt 等 Chat 模型特征
...
```
"晴" 的得分 = 输入向量 × 0.82 + (-0.15) = 3.7 "雨" 的得分 = 输入向量 × 0.41 + 0.23 = 1.2
```
...
答案已经在检索到的文本片段里了，模型只需要整理和表达，不需要复杂推理
...
![无法获取该图片](https://oss.open8gu.com/image-20260222133901342.png)
...
```
// 伪代码，展示 API 调用的基本概念 String answer = callLLM(    model: "Qwen/Qwen3-32B",    // 选择模型    systemPrompt: "你是一个电商客服助手",      // 系统提示词    userMessage: "买了一周的东西还能退吗？",    // 用户的问题    temperature: 0.1                         // 低温度，回答更准确 ); // answer = "根据我们的退货政策，自签收之日起7天内..."
```
...
```
curl https://api.siliconflow.cn/v1/chat/completions \  -H "Authorization: Bearer 你的API密钥" \  -H "Content-Type: application/json" \  -d '{    "model": "Qwen/Qwen2.5-7B-Instruct",    "messages": [     {"role": "system", "content": "你是一个电商客服助手"},     {"role": "user", "content": "买了一周的东西还能退吗？"}   ],    "temperature": 0.1 }'
```
...
> Source: https://t.zsxq.com/qlL8D
> Resolved: https://articles.zsxq.com/id_aebmicieg04w.html
